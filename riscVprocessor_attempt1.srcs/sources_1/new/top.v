...
//ALU
ALU ALunit(.A(readdata1_top_IDEX), .B(mux1out_top), .Control_in(ALUcontrol_top), .ALU_result(address_top), .zero(zero_top));
//ALU mux1
mux1 ALUmux(.sel1(ALUsrc_top_IDEX), .A1(readdata2_top_IDEX), .B1(IMMext_top_IDEX), .mux1_out(mux1out_top));
// Adder
...
EXMEM_reg EXMEMreg(.clk(clk), .reset(reset),
    .address_in(address_top), .readdata2_in(readdata2_top_IDEX), .instruction_in(instruction_top_IDEX),
